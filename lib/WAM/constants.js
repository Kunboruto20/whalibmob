...
// The WAM event and global tables, exactly as the reference client defines them.
//
// Ported verbatim from Baileys' WAM/constants.js — every event id, every field
// id and every type, unchanged. This is a generated table scraped from
// WhatsApp Web itself, not something to hand-edit: an id that drifts from what
// the server expects makes the whole buffer unparseable on the other side.
//
// WEB_EVENTS   one entry per event: { name, id, props: { field: [id, type] },
//              wamChannel, weight }
// WEB_GLOBALS  the attributes that ride alongside any event
// FLAG_*       the wire flags the encoder builds its headers from
...
module.exports = { WEB_EVENTS, WEB_GLOBALS, FLAG_BYTE, FLAG_GLOBAL, FLAG_EVENT, FLAG_FIELD, FLAG_EXTENDED };
