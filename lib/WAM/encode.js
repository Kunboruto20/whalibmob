...
// The WAM binary encoder, ported line for line from Baileys' WAM/encode.js.
//
// A buffer is a 'WAM' magic header, then a run of length-prefixed values whose
// flags say whether each one is a global attribute, an event, or a field of the
// event before it. Nothing here is guesswork — the layout is the reference
// client's and the ids come from constants.js.

const { BinaryInfo } = require('./BinaryInfo');
const { FLAG_BYTE, FLAG_EVENT, FLAG_EXTENDED, FLAG_FIELD, FLAG_GLOBAL,
        WEB_EVENTS, WEB_GLOBALS } = require('./constants');
