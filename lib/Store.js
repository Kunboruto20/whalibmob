...
function createNewStore(phoneNumber, opts) {
  ...
  return {
    ...
    simMcc:        opts.simMcc ? String(opts.simMcc).trim() : null,
    simMnc:        opts.simMnc ? String(opts.simMnc).trim() : null,
    version,
    device,
...
function storeFromJson(obj) {
  ...
  return {
    ...
    // Null on stores written before the SIM could be declared, which simply
    // means the country table's guess stands.
    simMcc:         obj.simMcc || null,
    simMnc:         obj.simMnc || null,
    codePending:    obj.codePending || false,
    codeMethod:     obj.codeMethod || null,
