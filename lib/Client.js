...
class WhalibmobClient extends EventEmitter {
  ...
  async _mexQuery(queryId, variables, dataPath) {
    const resp = await this._sendIq(new BinaryNode('iq', {
      ...
      new BinaryNode('query', { query_id: String(queryId) },
        Buffer.from(JSON.stringify({ variables: variables || {} }), 'utf8'))
    ]));
    ...
    if (!buf) throw new Error('mex query ' + queryId + ': reply carried no result');

    let json;
    try { json = JSON.parse(buf.toString('utf8')); }
    catch (_) { throw new Error('mex query ' + queryId + ': reply was not JSON'); }
