...
class WhalibmobClient extends EventEmitter {
  ...
  async _mexQuery(queryId, variables, dataPath) {
    ...
    const send = (askForJson) => {
      const attrs = { query_id: String(queryId) };
      // Newer servers answer this transport in Argo, Meta's binary GraphQL
      // encoding, which nothing here can read. The reply labels its own format,
      // so the format is evidently negotiable — ask for the one we can parse.
      // A server that does not know the attribute ignores it, and one that
      // refuses the stanza over it is handled below.
      if (askForJson) attrs.format = 'json';
    ...
    // The query itself worked — this is purely about how the answer was
    // encoded, and saying so is the difference between a fixable report and a
    // shrug.
    ...
    if (format && String(format).toLowerCase() !== 'json') {
      ...
      const err = new Error('mex query ' + queryId + ': the server answered in "' +
        format + '" rather than JSON, which this library cannot read' +
