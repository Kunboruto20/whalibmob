...
// WhatsApp newsletter (channel) GraphQL persisted-query IDs. These rotate on
// the server side; stale IDs make the query fail outright, which is why the
// previous hardcoded set stopped working. Kept together so they are easy to
...
class WhalibmobClient extends EventEmitter {
  constructor(opts) {
    ...
    this._groupDescId      = new Map();   // groupJid → current description id
  ...
  _parseMediaConn(node) {
    const connNode = findChild(node, 'media_conn');
    if (!connNode) return null;
    const auth  = (connNode.attrs && connNode.attrs.auth) || '';
    const ttl   = connNode.attrs && connNode.attrs.ttl ? parseInt(connNode.attrs.ttl, 10) : 0;
    const hosts = [];
    ...
    return { hosts, auth, ttl, fetchDate: Date.now() };
  }

  _onMediaConnectionResult(node) {
    const conn = this._parseMediaConn(node);
    if (!conn) return;
    this._mediaConn = conn;
    if (this._sender) this._sender.setMediaConnection(conn.hosts, conn.auth);
    this.emit('media_conn', conn);
  }

  // Whether the cached media connection is still within its server-provided TTL.
  // The auth token embedded in the media connection expires; using a stale one
  // makes uploads hang until they time out ("connection expired"). A missing
  // ttl is treated conservatively with a short default window.
  _isMediaConnFresh() {
    const c = this._mediaConn;
    if (!c || !c.hosts || c.hosts.length === 0 || !c.auth) return false;
    const ttlMs = ((c.ttl && c.ttl > 0) ? c.ttl : 300) * 1000; // default 5 min
    return (Date.now() - c.fetchDate) < ttlMs;
  }

  // Return a usable media connection, re-fetching when stale or when forced.
  // forceGet=true is used after an upload failure to rotate a dead auth token.
  async refreshMediaConn(forceGet) {
    if (!forceGet && this._isMediaConnFresh()) return this._mediaConn;

    const id   = this._genMsgId();
    const node = new BinaryNode('iq', {
      id, to: 's.whatsapp.net', type: 'set', xmlns: 'w:m'
    }, [new BinaryNode('media_conn', {}, null)]);

    const resp = await this._sendIq(node).catch(() => null);
    const conn = resp ? this._parseMediaConn(resp) : null;
    if (conn && conn.hosts.length > 0) {
      this._mediaConn = conn;
      if (this._sender) this._sender.setMediaConnection(conn.hosts, conn.auth);
      this.emit('media_conn', conn);
      return conn;
    ...
    // Fall back to whatever we already had rather than failing outright.
    if (this._mediaConn && this._mediaConn.hosts && this._mediaConn.hosts.length) {
      return this._mediaConn;
    }
    throw new Error('Failed to obtain media connection');
  }
  ...
  _parseGroupNode(groupNode) {
    ...
    // description — descId is required as the `prev` attribute when replacing
    // an existing description, so it has to survive parsing.
    const descNode = children.find(n => n && n.description === 'description');
    let description = null;
    const descId = descNode && descNode.attrs ? (descNode.attrs.id || null) : null;
    if (descNode) {
    ...
    // update member cache, addressing mode and description id
    this._groupMembers.set(jid, new Set(participants.map(p => p.jid)));
    this._groupAddressingMode.set(jid, addressingMode);
    if (descId) this._groupDescId.set(jid, descId);
    else this._groupDescId.delete(jid);
  ...
  async fetchAllGroups() {
    ...
    // The server wraps the results in a <groups> node:
    //   <iq><groups><group .../><group .../></groups></iq>
    // Reading <group> straight off the <iq> finds nothing and silently
    // returns an empty list, which made fetchAllGroups always look like the
    // account had no groups (and left the group member cache empty, so group
    // sends had no participants to fan out to).
  ...
  //
  // The chatstate must carry `from` (our own JID) so the server can route the
  // indicator; without it WhatsApp drops the typing/recording state. LID chats
  // use our LID identity as `from`, matching the conversation's addressing.
  setChatPresence(chatJid, presence) {
    if (!this._socket || !this._connected) return;
    const jid   = makeJid(chatJid);
    const isLid = jid.endsWith('@lid');
    const from  = (isLid && this._myLid)
      ? this._myLid
      : `${String(this._store.phoneNumber)}@s.whatsapp.net`;

    if (presence === 'composing' || presence === 'recording') {
      const attrs = {};
      if (presence === 'recording') attrs.media = 'audio';
      this._socket.sendNode(new BinaryNode('chatstate', { from, to: jid }, [
  ...
  // Internal: run a w:mex GraphQL query and return data.<dataPath> (or all of
  // `data` when dataPath is omitted). Surfaces server-side GraphQL errors
  // instead of silently returning null.
  async _newsletterQuery(queryId, variables, dataPath) {
    const id   = this._genMsgId();
    const body = JSON.stringify({ variables });
    const node = new BinaryNode('iq', {
      id, xmlns: 'w:mex', to: 's.whatsapp.net', type: 'get'
    }, [new BinaryNode('query', { query_id: queryId }, Buffer.from(body, 'utf8'))]);

    const resp = await this._sendIq(node);
    if (!resp) return null;
    const resultNode = findChild(resp, 'result');
    const buf = resultNode ? getNodeContent(resultNode) : null;
    if (!buf) return null;

    let json;
    try { json = JSON.parse(buf.toString('utf8')); } catch (_) { return null; }
    if (json.errors && json.errors.length) {
      const msg = json.errors.map(e => (e && e.message) || 'error').join(', ');
      throw new Error('Newsletter query failed: ' + msg);
    }
    const data = json.data || {};
    if (!dataPath) return data;
    return data[dataPath] !== undefined ? data[dataPath] : null;
  }

  // Normalise a newsletter thread object (create + metadata share this shape).
  _parseNewsletterThread(data) {
    if (!data || typeof data !== 'object') return null;
    const nl     = (data.result && typeof data.result === 'object') ? data.result : data;
    const thread = nl.thread_metadata || nl;
    const textOf = (v) => (v && typeof v === 'object' && 'text' in v) ? v.text : (v == null ? null : v);
    return {
      jid:             nl.id || null,
      name:            textOf(thread.name),
      description:     textOf(thread.description),
      subscriberCount: parseInt(thread.subscribers_count, 10) || 0,
      creationTime:    parseInt(thread.creation_time, 10) || 0,
      invite:          thread.invite || null,
      verification:    thread.verification || null,
      muteState:       (nl.viewer_metadata && nl.viewer_metadata.mute) || null
    };
  }

  // Create a newsletter / channel. Returns the parsed thread or null.
  async createNewsletter(name, description) {
    const data = await this._newsletterQuery(
      NEWSLETTER_QUERY_IDS.CREATE,
      { input: { name: name || '', description: description == null ? null : description } },
      'xwa2_newsletter_create'
    );
    return this._parseNewsletterThread(data);
  }

  // Query newsletter metadata by JID. Returns the parsed thread or null.
  ...
  // Join / follow a newsletter
  async joinNewsletter(newsletterJid) {
    const jid = makeJid(newsletterJid, 'newsletter');
    return this._newsletterQuery(NEWSLETTER_QUERY_IDS.FOLLOW, { newsletter_id: jid }, 'xwa2_newsletter_join_v2');
  }

  // Leave / unfollow a newsletter
  async leaveNewsletter(newsletterJid) {
    const jid = makeJid(newsletterJid, 'newsletter');
    return this._newsletterQuery(NEWSLETTER_QUERY_IDS.UNFOLLOW, { newsletter_id: jid }, 'xwa2_newsletter_leave_v2');
  }

  // Mute a newsletter
  async muteNewsletter(newsletterJid) {
    const jid = makeJid(newsletterJid, 'newsletter');
    return this._newsletterQuery(NEWSLETTER_QUERY_IDS.MUTE, { newsletter_id: jid }, 'xwa2_newsletter_mute_v2');
  }

  // Unmute a newsletter
  async unmuteNewsletter(newsletterJid) {
    const jid = makeJid(newsletterJid, 'newsletter');
    return this._newsletterQuery(NEWSLETTER_QUERY_IDS.UNMUTE, { newsletter_id: jid }, 'xwa2_newsletter_unmute_v2');
  }

  // Fetch a newsletter's subscriber list
  async newsletterSubscribers(newsletterJid) {
    const jid = makeJid(newsletterJid, 'newsletter');
    return this._newsletterQuery(NEWSLETTER_QUERY_IDS.SUBSCRIBERS, { newsletter_id: jid }, 'xwa2_newsletter_subscribers');
  }

  // Delete a newsletter you own
  async deleteNewsletter(newsletterJid) {
    const jid = makeJid(newsletterJid, 'newsletter');
    return this._newsletterQuery(NEWSLETTER_QUERY_IDS.DELETE, { newsletter_id: jid }, 'xwa2_newsletter_delete_v2');
  }

  // Change newsletter description
  async changeNewsletterDescription(newsletterJid, description) {
    const jid = makeJid(newsletterJid, 'newsletter');
    return this._newsletterQuery(
      NEWSLETTER_QUERY_IDS.UPDATE_METADATA,
      { newsletter_id: jid, updates: { description: description || '', settings: null } },
      'xwa2_newsletter_update'
    );
  }

  // Change newsletter name
  async changeNewsletterName(newsletterJid, name) {
    const jid = makeJid(newsletterJid, 'newsletter');
    return this._newsletterQuery(
      NEWSLETTER_QUERY_IDS.UPDATE_METADATA,
      { newsletter_id: jid, updates: { name: name || '', settings: null } },
      'xwa2_newsletter_update'
    );
  }
  ...
  async changeGroupDescription(groupJid, description) {
    ...
    // Replacing an existing description requires `prev` — the id of the
    // description currently on the server. Without it the update is rejected.
    // Refresh the metadata when we don't already know the current id.
    let prev = this._groupDescId.get(gJid) || null;
    if (!prev) {
      try { await this.getGroupMetadata(gJid); } catch (_) {}
      prev = this._groupDescId.get(gJid) || null;
    }
