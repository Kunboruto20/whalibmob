...
class WhalibmobClient extends EventEmitter {
  ...
  async requestPairingCode(phoneNumber, customCode) {
    ...
    const node = new BinaryNode('iq', {
      ...
      new BinaryNode('link_code_companion_reg', {
        ...
        new BinaryNode('companion_server_auth_key_pub', {}, this._store.noiseKeyPair.public),
  ...
  _handleIq(node) {
    ...
    if (this._mode === 'web' && type === 'set') {
      if (findChild(node, 'pair-device'))  { this._handlePairDevice(node);  return; }
