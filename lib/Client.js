...
class WhalibmobClient extends EventEmitter {
  ...
  async init(phoneNumber) {
    ...
    // A fresh attempt clears whatever made the last one fatal. Only the public
  ...
  async _handleCompanionFinish(node) {
    ...
    try {
      ...
      // Baileys awaits this one rather than firing and forgetting, so a server
      // rejection surfaces as an error here instead of as a silent stall
      // waiting for a pair-success that is never coming.
  ...
  _handleIq(node) {
    ...
    // the way Baileys distinguishes them ('CB:iq,type:set,pair-device' against
    // a bare 'CB:iq,,pair-success'). Requiring type=set on pair-success would
    // drop the stanza that completes the link.
    if (this._mode === 'web') {
  ...
  //
  // Both references resolve it to the contact's LID and fall back to the plain
  // JID only when no LID is known — whatsmeow's resolveTCTokenStorageLID and
  // Baileys' resolveTcTokenJid are the same three lines:
  //
  ...
  // names the phone JID. Baileys implements exactly this in resolveIssuanceJid
  // and defaults the prop to false; whatsmeow issues to the LID instead. We
  // follow WA Web, and more importantly we now follow it in one place: the
  ...
  //
  // whatsmeow funnels every attach site through ensureTCToken, which returns
  // nothing for an expired token. whalibmob had the expiry check on the message
  // path only; presence subscribe, group create, participant add and the
  ...
  _handleFailure(node) {
    ...
    // Not every failure kills the session. whatsmeow's IsLoggedOut() marks the
    // ones that do — 401, 403, 405, 406, 409 — and treats the rest as worth
    // another attempt. A 500 or a 503 is the server having a bad minute; giving
    ...
    if (result) {
      try {
        ...
        for (const tokenNode of entries) {
          ...
          // Baileys drops anything that is not a regular user here — the guard
          // is against a malformed or spoofed notification filing a token under
  ...
  _reissueTcTokenAfterIdentityChange(from) {
    ...
    void (async () => {
      try {
        ...
        // Baileys re-issues with the *previous* senderTimestamp, not a fresh
        // one, and stores the result without touching the dedupe state. That
  ...
  subscribeToPresence(jids) {
    ...
    for (const raw of arr) {
      ...
      // Groups and newsletters have no privacy token; Baileys guards the same
      // way before building the node.
  ...
  async markChatRead(jid) {
    const jidFull = makeJid(jid);
    await this._sendAppStatePatch(AppMutations.markReadPatch(jidFull, true));
  ...
  async markChatUnread(jid) {
    const jidFull = makeJid(jid);
    await this._sendAppStatePatch(AppMutations.markReadPatch(jidFull, false));
    this._getChatState(jidFull).markedAsUnread = true;
    this.emit('chat_read', { jid: jidFull, read: false });
    return true;
  }
  ...
  async muteChat(jid, durationMs) {
    ...
    await this._sendAppStatePatch(AppMutations.mutePatch(jidFull, endMs || 1));
  ...
  async unmuteChat(jid) {
    const jidFull = makeJid(jid);
    await this._sendAppStatePatch(AppMutations.mutePatch(jidFull, 0));
    const chat = this._getChatState(jidFull);
    chat.muted       = false;
    chat.muteUntilMs = 0;
  ...
  // it; an unblock carries only the LID. That is the shape Baileys'
  // updateBlockStatus builds, and the one this server accepts — whatsmeow's
  // UpdateBlocklist still sends the plain JID it was handed, which is what this
  // used to do and what the server answered with 400 bad-request.
  //
  ...
  async pinChat(jid) {
    const jidFull = makeJid(jid);
    await this._sendAppStatePatch(AppMutations.pinPatch(jidFull, true));
    this._getChatState(jidFull).pinnedAt = Math.floor(Date.now() / 1000);
    this.emit('chat_pinned', { jid: jidFull, pinned: true });
    return true;
  }

  async unpinChat(jid) {
    const jidFull = makeJid(jid);
    await this._sendAppStatePatch(AppMutations.pinPatch(jidFull, false));
    this._getChatState(jidFull).pinnedAt = 0;
    this.emit('chat_pinned', { jid: jidFull, pinned: false });
    return true;
  }

  async archiveChat(jid) {
    const jidFull = makeJid(jid);
    await this._sendAppStatePatch(AppMutations.archivePatch(jidFull, true));
    this._getChatState(jidFull).archived = true;
    this.emit('chat_archived', { jid: jidFull, archived: true });
    return true;
  }

  async unarchiveChat(jid) {
    const jidFull = makeJid(jid);
    await this._sendAppStatePatch(AppMutations.archivePatch(jidFull, false));
    this._getChatState(jidFull).archived = false;
    this.emit('chat_archived', { jid: jidFull, archived: false });
    return true;
  }
  ...
  async _setStar(msgId, chatJid, fromMe, starred) {
    ...
    await this._sendAppStatePatch(
      AppMutations.starPatch(jidFull, msgId, !!fromMe, starred));
    if (!this._starredMessages) this._starredMessages = new Set();
    const key = jidFull + ':' + msgId;
    if (starred) this._starredMessages.add(key); else this._starredMessages.delete(key);
    this.emit('message_starred', { msgId, chatJid: jidFull, starred, fromMe: !!fromMe });
    return true;
  }
