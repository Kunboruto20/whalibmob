...
class WhalibmobClient extends EventEmitter {
  ...
  changeName(name) {
    ...
    // Availability is a separate statement, and one this method used to make as
    // a side effect. Callers came to rely on that — a session that refreshes its
    // name on a timer is also holding its presence open — so it is still made,
    // but only when the account is already marked available, and never as the
    // thing that carries the rename.
    if (this._sendActiveReceipts) {
      this._socket.sendNode(new BinaryNode('presence', {
        name,
        type: 'available'
      }, null));
