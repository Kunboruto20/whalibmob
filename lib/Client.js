...
class WhalibmobClient extends EventEmitter {
  ...
  async checkSessionAlive() {
    ...
    try {
      ...
      return { alive: status === 'ok', status, raw: res };
    } catch (err) {
  ...
  _handleStreamError(node) {
    ...
    const reason = attrs.reason || code || 'unknown';

    if (code === '515') {
      _whaDbg('[DBG] STREAM_ERROR 515 — restart required after pairing, reconnecting');
      this.emit('restart_required', { reason });
      return;
    }

    _whaDbg('[DBG] STREAM_ERROR code=' + code + ' reason=' + reason);
    this.emit('stream_error', { reason, code });
  }
