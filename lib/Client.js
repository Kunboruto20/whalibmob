...
const { checkIfRegistered, checkNumberStatus, requestSmsCode, verifyCode, assertRegistrationKeys, fetchIosVersion, fetchWaVersion, resolveConnectVersion, clearVersionCache } = require('./Registration');
...
class WhalibmobClient extends EventEmitter {
  ...
  async init(phoneNumber) {
    ...
    // Say what version this is before saying hello, rather than repeating
    // whatever the registration recorded until the server stops accepting it.
    await this._refreshAnnouncedVersion();

    try {
      await this._connectSocket();
    } catch (err) {
      // 405 is the server refusing the version we just announced. The lookup
      // that produced it is memoised, so the cached answer is precisely the one
      // that was refused — drop it, ask again, and try once more. Only once:
      // a second 405 on a freshly fetched version is not about staleness.
      if (String(err && err.code) === '405' && !this._versionRetried) {
        this._versionRetried = true;
        const refused = this._store.version;
        clearVersionCache();
        await this._refreshAnnouncedVersion();
        if (this._store.version !== refused) {
          _whaDbg('[DBG] VERSION 405 on ' + refused + ' — retrying as ' + this._store.version);
          this._fatal = false;
          await this._connectSocket();
          this._persistAnnouncedVersion(sessionFile);
          return this;
        }
      }
  ...
  /**
   * Point the session's version at whatever the platform currently ships.
   *
   * Held in memory only until the connection succeeds — a lookup that returns
   * something the server will not take should not be what the session carries
   * from then on. _persistAnnouncedVersion writes it once the handshake has
   * proved it works.
   *
   * Never throws: the session's own version is a perfectly good answer, and is
   * what an offline run keeps using.
   */
  async _refreshAnnouncedVersion() {
    if (!this._store) return;
    try {
      const version = await resolveConnectVersion(this._store);
      if (version && version !== this._store.version) {
        _whaDbg('[DBG] VERSION announce ' + this._store.version + ' → ' + version);
        this._store.version   = version;
        this._versionChanged  = true;
      }
    } catch (err) {
      _whaDbg('[DBG] VERSION lookup failed, keeping ' +
        (this._store && this._store.version) + ': ' + (err && err.message));
    }
  }

  // Keep a version the server has just accepted, so the next run starts from it
  // even with no network to look one up.
  _persistAnnouncedVersion(sessionFile) {
    if (!this._versionChanged || !this._store) return;
    this._versionChanged = false;
    try { saveStore(this._store, sessionFile); } catch (_) {}
  }
