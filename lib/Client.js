...
class WhalibmobClient extends EventEmitter {
  ...
  //
  // The six-digit PIN under Settings → Account → Two-step verification, and the
  // recovery email that goes with it.
  //
  // READ THIS BEFORE USING THE WRITE PATH.
  //
  // No open-source client implements setting this — not Baileys, not whatsmeow,
  // not Cobalt. There is no reference wire format to copy, so the structure
  // below is inferred, not verified against a real client. What is actually
  // known is narrow: `urn:xmpp:whatsapp:account` is a real namespace (it has its
  // own entry in the protocol's token dictionary) and `code` and `email` are
  // real tokens. The child structure is a reasoned guess.
  //
  // The cost of that guess landing wrong is not abstract. WhatsApp makes you
  // wait seven days to re-register a number whose PIN you cannot produce, so a
  // write that half-succeeds takes the number out for a week. Every call here
  // reports the server's own reply verbatim precisely so you can tell which
  // happened instead of assuming.
  //
  // Both halves of a number can use these: the setting belongs to the account
  // rather than to a device, and this travels as an ordinary IQ on whichever
  // socket is authenticated — so an SMS session and a companion behave alike.

  /**
   * Read whether two-step verification is on, and the recovery email if set.
   *
   * The server's own reply is handed back in `raw` alongside the parsed fields,
   * because the exact shape of this response is not documented anywhere and a
   * caller may well see something these fields do not cover.
   *
   * An error is reported rather than thrown: a server that does not answer this
   * query is itself the useful answer, and callers should be able to see the
   * code without a try/catch around a feature probe.
   *
   * @returns {Promise<{supported: boolean, enabled: boolean|null,
   *                    email: string|null, error: string|null, raw: any}>}
   */
  async queryTwoStep() {
    const node = new BinaryNode('iq', {
      id:    this._genMsgId(),
      ...
      to:    's.whatsapp.net',
      type:  'get'
    }, [new BinaryNode('2fa', {}, null)]);

    let resp;
    try {
      resp = await this._sendIq(node);
    } catch (e) {
      return { supported: false, enabled: null, email: null,
               error: (e && e.message) || String(e), raw: null };
    }
    if (!resp) {
      return { supported: false, enabled: null, email: null,
               error: 'no reply from server (IQ timed out)', raw: null };
    }
    if (resp.attrs && resp.attrs.type === 'error') {
      const errNode = findChild(resp, 'error');
      const code    = errNode && errNode.attrs && errNode.attrs.code;
      ...
      return {
        supported: false, enabled: null, email: null,
        error: 'server rejected the query' +
               (code ? ' (' + code + (text ? ' ' + text : '') + ')' : ''),
        raw: resp
      };
    }

    // The reply's shape is the server's to choose. Read what is recognisable
    // and hand the rest back untouched rather than inventing a schema.
    const twoFa = findChild(resp, '2fa') || resp;
    let enabled = null, email = null;
    if (twoFa && Array.isArray(twoFa.content)) {
      for (const child of twoFa.content) {
        if (!child) continue;
        const tag = child.description;
        if (tag === 'email') email = getNodeContent(child) ? String(getNodeContent(child)) : null;
        if (tag === 'code' || tag === 'pin') enabled = true;
      }
      if (enabled === null) enabled = twoFa.content.length > 0;
    }
    if (twoFa && twoFa.attrs) {
      if (twoFa.attrs.email) email = String(twoFa.attrs.email);
      if (twoFa.attrs.enabled !== undefined) enabled = twoFa.attrs.enabled === 'true';
    }

    return { supported: true, enabled, email, error: null, raw: resp };
  }

  /**
   * Ask the server the same question several ways, and report what each is
   * answered with.
   *
   * A single error code says very little on its own. `403 forbidden` could mean
   * the server understood the request and refused it on authority, or it could
   * be what this server says to anything it does not recognise. The difference
   * matters — one says the format is right and the account or device is not
   * allowed, the other says the format is wrong — and the only way to tell them
   * apart is to ask something deliberately nonsense and see whether the answer
   * changes.
   *
   * Every probe is a `get`. Nothing here changes anything.
   *
   * @returns {Promise<Array<{name: string, xmlns: string, child: string|null,
   *                          code: string|null, text: string|null, ok: boolean}>>}
   */
  async probeTwoStep() {
    const probes = [
      // What we actually send.
      { name: 'ours',            xmlns: 'urn:xmpp:whatsapp:account', child: '2fa'  },
      // Same namespace, no child — separates "namespace refused" from
      // "namespace fine, that child is not a thing".
      { name: 'ours, no child',  xmlns: 'urn:xmpp:whatsapp:account', child: null   },
      // Same namespace, other plausible spellings of the child.
      { name: 'ours, <account>', xmlns: 'urn:xmpp:whatsapp:account', child: 'account' },
      { name: 'ours, <2fa_v2>',  xmlns: 'urn:xmpp:whatsapp:account', child: '2fa_v2'  },
      // A namespace that certainly does not exist. Whatever comes back is this
      // server's word for "never heard of it", and it is the baseline every
      // other answer is read against.
      { name: 'nonsense ns',     xmlns: 'urn:xmpp:whatsapp:nonexistent', child: '2fa' },
      // A namespace that certainly does exist and we are certainly allowed to
      // use. Proves the probe itself works and the connection is healthy.
      { name: 'known-good (privacy)', xmlns: 'privacy', child: 'privacy' }
    ];
    ...
    for (const p of probes) {
      const node = new BinaryNode('iq', {
        ...
        to:    's.whatsapp.net',
        type:  'get'
      }, p.child ? [new BinaryNode(p.child, {}, null)] : null);

      let code = null, text = null, ok = false;
      try {
        const resp = await this._sendIq(node);
        if (!resp) { text = 'timed out'; }
        else if (resp.attrs && resp.attrs.type === 'error') {
          const e = findChild(resp, 'error');
          code = e && e.attrs && e.attrs.code ? String(e.attrs.code) : null;
          text = e && e.attrs && e.attrs.text ? String(e.attrs.text) : null;
        } else ok = true;
      } catch (err) {
        text = (err && err.message) || String(err);
      }
      out.push({ name: p.name, xmlns: p.xmlns, child: p.child, code, text, ok });
    }
    return out;
  }

  // The one place the two-step IQ is built, so set and remove cannot drift into
  // sending differently-shaped stanzas for the same setting.
  async _sendTwoStepIq(children) {
    if (!this._socket || !this._connected) throw new Error('Not connected');
    ...
    const node = new BinaryNode('iq', {
      ...
      to:    's.whatsapp.net',
      type:  'set'
    }, [new BinaryNode('2fa', {}, children)]);

    const resp = await this._sendIq(node);
    if (!resp) {
      const err = new Error('two-step verification: no reply from server (IQ timed out)');
      err.raw = null;
      throw err;
    }
    if (resp.attrs && resp.attrs.type === 'error') {
      ...
      const code    = errNode && errNode.attrs && errNode.attrs.code;
      const text    = errNode && errNode.attrs && errNode.attrs.text;
      const err = new Error('two-step verification: server rejected the change' +
        (code ? ' (' + code + (text ? ' ' + text : '') + ')' : '') +
        '. The wire format for this is inferred rather than verified — the ' +
        'server\'s reply is on err.raw.');
      err.code = code ? String(code) : null;
      err.raw  = resp;
      throw err;
    }
    return resp;
  ...
  /**
   * Turn two-step verification on, or change the PIN that is already set.
   *
   * The PIN is what WhatsApp will demand the next time this number is
   * registered anywhere. Set one you can produce — a number whose PIN is lost
   * cannot be registered for seven days.
   *
   * The recovery email is optional and is what WhatsApp uses to let you reset a
   * forgotten PIN. Without one, a forgotten PIN has no way back but the wait.
   *
   * @param {string} pin           exactly six digits
   * @param {string} [email]       recovery address, or omitted to leave it alone
   * @returns {Promise<{ok: true, raw: any}>} the server's own reply, verbatim
   */
  async setTwoStep(pin, email) {
    const digits = String(pin == null ? '' : pin).trim();
    if (!/^\d{6}$/.test(digits)) {
      throw new Error('setTwoStep: the PIN must be exactly six digits, got ' +
        (digits ? '"' + digits + '" (' + digits.length + ')' : 'nothing'));
    }
    if (email != null) {
      const addr = String(email).trim();
      // Not a validator — just enough to catch a phone number or a name typed
      // into the wrong argument, which the server would answer with a bare 400.
      if (addr && !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(addr)) {
        throw new Error('setTwoStep: "' + addr + '" does not look like an email address');
      }
    }

    const children = [new BinaryNode('code', {}, Buffer.from(digits, 'utf8'))];
    if (email != null && String(email).trim()) {
      children.push(new BinaryNode('email', {}, Buffer.from(String(email).trim(), 'utf8')));
    }

    const resp = await this._sendTwoStepIq(children);
    _whaDbg('[DBG] TWO_STEP_SET pin=****** email=' + (email ? String(email) : 'unchanged'));
   ...
   *
   * Sent as an empty `<code/>`, which is how this protocol clears a value
   * elsewhere — a removal is the same stanza with nothing in it rather than a
   * verb of its own.
   *
   * @returns {Promise<{ok: true, raw: any}>} the server's own reply, verbatim
   */
  async removeTwoStep() {
    const resp = await this._sendTwoStepIq([new BinaryNode('code', {}, null)]);
    _whaDbg('[DBG] TWO_STEP_REMOVE');
    return { ok: true, raw: resp };
  }
