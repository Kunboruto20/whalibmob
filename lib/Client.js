...
class WhalibmobClient extends EventEmitter {
  ...
  _onNode(node) {
    ...
    try {
      let result;
      if      (tag === 'iq')           result = this._handleIq(node);
      else if (tag === 'message')      result = this._handleMessage(node);
      else if (tag === 'receipt')      result = this._handleReceipt(node);
      else if (tag === 'ack')          result = this._handleAck(node);
      else if (tag === 'notification') result = this._handleNotification(node);
      else if (tag === 'presence')     result = this._handlePresence(node);
      else if (tag === 'call')         result = this._handleCall(node);
      else if (tag === 'ib')           result = this._handleIb(node);
      else if (tag === 'success')      this.emit('session_refresh', { node }); // late success (re-auth)
      else if (tag === 'failure')      result = this._handleFailure(node);
      else if (tag === 'stream:error') result = this._handleStreamError(node);
      else                             this.emit('node', node);

      // An async handler does not throw — it returns a promise that rejects,
      // and the catch below has already been passed by the time it does. So the
      // containment above covered only the handlers that happen to be
      // synchronous: _handleMessage is async, and a fault inside it escaped as
      // an unhandled rejection, which says nothing about which stanza did it
      // and ends the process outright on Node 15 and later.
  ...
  async _handleMessage(node) {
    ...
    // The acknowledgement goes out before anything is read out of the stanza,
    // and stays outside the queues below.
    //
    // It says one thing — this arrived — and that is already true here. Nothing
    // further on can make it untrue, so nothing further on gets to prevent it:
    // it used to sit after the parsing, and a stanza shaped in a way the parsing
    // did not expect went unacknowledged, which the server reads as undelivered
    // and the sender sees as a single tick. It has to be prompt whatever the
    // decryption goes on to do, too — holding it behind a queue of other
    ...
    // A message fanned out per device carries no <enc> of its own: each
    // recipient device gets its own inside <participants>, and ours has to be
    // picked out of them.
    //
    // A group message is not that shape and must not come through here. It
    // carries the same block for a different reason — the sender key rides in
    // it — and the branch below reads it properly.
    //
    // Both names this account answers to are matched. The server addresses
    // devices by LID as readily as by number, and a <to> naming our LID is just
    // as much ours; comparing against the phone number alone left the fan-out
    // unclaimed and the message unread.
    //
    // And the JID is stringified before it is taken apart. A decoded JID
    // attribute is an object — BinaryNode builds { user, agent, device, server }
    // with a toString(), not a string — so calling .split() on it threw
    // "toJid.split is not a function". That throw landed before the ack below,
    // which is the whole of this: the stanza arrived, nothing acknowledged it,
    // and the sender was left on one tick with the message unread and the group
    // sender key never stored. _processSKDMDistribution has always stringified
    // it; this side never did.
    if (!dmEncNode && !(skmsgNode && isGroup) &&
        participantsNode && Array.isArray(participantsNode.content)) {
      const ownPhone   = String((this._store && this._store.phoneNumber) || '');
      const ownLidUser = this._myLid
        ? String(this._myLid).split('@')[0].split(':')[0]
        : null;
      for (const toNode of participantsNode.content) {
        ...
        if (toUser === ownPhone || (ownLidUser && toUser === ownLidUser)) {
