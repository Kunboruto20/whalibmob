...
class SessionBuilder {
    ...
    async initOutgoing(device) {
        ...
        return await queueJob(fqAddr, async () => {
            ...
            // The signed pre-key is what ties this bundle to the identity key we
            // just decided to trust. Taking it unverified would let anyone who
            // can answer for this address hand us a key of their choosing.
            if (!curve.verifySignature(device.identityKey, device.signedPreKey.publicKey,
                                       device.signedPreKey.signature)) {
                throw new errors.PreKeyError('Signed pre-key signature does not verify against the identity key');
            }
