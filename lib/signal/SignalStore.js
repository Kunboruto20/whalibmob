...
// named one per id:
//
//   <base>/919634847671/919634847671.pre-key-1.json
//   ...
//   <base>/919634847671/919634847671.pre-key-812.json
//
// Everything else about a pre-key is small, but there are 812 of them
// and every one that is consumed rewrites the whole store — so keeping them in
// the aggregate file meant a ~95 KB write on each incoming pkmsg, and made the
// pool something you could only inspect by parsing the file. A file each is
// what the reference client does, it is what anyone comparing the two
// authentication folders expects to see, and a write now touches one key.
//
// The stem in front of `.pre-key-` is the one the .signal.json file already
// carries, so a number's mobile half (919634847671.pre-key-N.json) and its
// companion half (919634847671.web.pre-key-N.json) keep separate pools, as they
...
//
// In memory whalibmob carries the 33-byte 0x05-prefixed public key libsignal
// hands out, so the prefix comes off on the way to disk and goes back on on the
// way in.
//
// Unlike the snapshot these are written straight, without the temp-file dance.
// The blast radius of a torn write is one key: it fails to parse at load, is
// skipped, and the pool is topped back up. The snapshot cannot be treated that
// way because losing it loses every session the account has.

const PRE_KEY_INFIX      = '.pre-key-';
const SIGNAL_KEY_PREFIX  = 0x05;

// How many pre-key files may be in flight at once. Eight hundred parallel
// writes is a reliable way to meet EMFILE.
const PRE_KEY_WRITE_CONCURRENCY = 32;

function _escapeRe(s) {
  return String(s).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
}

// A BufferJSON field, in any of the shapes it is written in.
function _bufFrom(field) {
  ...
  if (field.type === 'Buffer') {
    return Array.isArray(field.data)
      ...
      : Buffer.from(String(field.data), 'base64');
  }
  return null;
}

function _encodePreKeyFile(entry) {
  const pub = Buffer.from(entry.pubKey, 'base64');
  const raw = (pub.length === 33 && pub[0] === SIGNAL_KEY_PREFIX) ? pub.slice(1) : pub;
  return JSON.stringify({
    private: { type: 'Buffer', data: entry.privKey },
    public:  { type: 'Buffer', data: raw.toString('base64') }
  });
}

function _decodePreKeyFile(text) {
  const obj  = JSON.parse(text);
  const priv = _bufFrom(obj && obj.private);
  let   pub  = _bufFrom(obj && obj.public);
  if (!priv || !pub || priv.length !== 32) return null;
  if (pub.length === 32) pub = Buffer.concat([Buffer.from([SIGNAL_KEY_PREFIX]), pub]);
  if (pub.length !== 33) return null;
  return { pubKey: pub.toString('base64'), privKey: priv.toString('base64') };
}

// A stored counter, or 1 when the file predates it or holds nonsense.
function _counter(value) {
...
class SignalStore {
  constructor() {
    ...
    // Where the pre-key files go, and which ids are waiting to be written or
    // unlinked. Empty until attachFile() — a store with nowhere to write keeps
    // its pre-keys in memory and queues nothing.
    this._pkDir            = null;
    this._pkPrefix         = null;
    this._pkPending        = new Set();
    this._pkTimer          = null;
    ...
    //
    //   _nextPreKeyId             the id the next generated key takes
    //   _firstUnuploadedPreKeyId  the lowest id the server has not been sent
    //
    // Both only ever go up. An id that has been handed to the server is spent
    // even after the key itself is consumed and deleted, so deriving the next
    // id from what is still on disk — which is what this used to do — walks
    // backwards over ids the server is still serving and hands two different
    // keys out under one id.
    this._nextPreKeyId            = 1;
    this._firstUnuploadedPreKeyId = 1;
  ...
  detach() {
    ...
    this._filePath   = null;
    this._writeAgain = false;
    // Same for the pre-key files: _flushSync() above has written whatever was
    // outstanding, so let the directory go before the caller moves it.
    if (this._pkTimer) { clearTimeout(this._pkTimer); this._pkTimer = null; }
    this._pkDir    = null;
    this._pkPrefix = null;
    this._pkPending.clear();
  ...
  attachFile(filePath) {
    ...
    // The pre-key files sit beside the snapshot and share its stem, so
    // 919634847671.signal.json gives 919634847671.pre-key-<id>.json and
    // 919634847671.web.signal.json gives 919634847671.web.pre-key-<id>.json.
    const base = path.basename(filePath);
    this._pkDir    = path.dirname(filePath);
    this._pkPrefix = base.endsWith('.signal.json')
      ? base.slice(0, -'.signal.json'.length)
      : base.replace(/\.json$/, '');
    this._load();
  }

  /** Where a pre-key of this id belongs on disk. */
  _preKeyFile(id) {
    return path.join(this._pkDir, this._pkPrefix + PRE_KEY_INFIX + id + '.json');
  }

  // Read every pre-key file this session owns. Returns how many were read.
  //
  // A file that will not parse is skipped rather than fatal: it costs one key
  // out of 812, and the pool is topped back up on the next check.
  _loadPreKeyFiles() {
    if (!this._pkDir) return 0;
    let entries;
    try { entries = fs.readdirSync(this._pkDir); } catch (_) { return 0; }

    const re = new RegExp('^' + _escapeRe(this._pkPrefix + PRE_KEY_INFIX) + '(\\d+)\\.json$');
    let count = 0;
    for (const name of entries) {
      const m = re.exec(name);
      if (!m) continue;
      try {
        const entry = _decodePreKeyFile(
          fs.readFileSync(path.join(this._pkDir, name), 'utf8'));
        if (entry) { this._preKeys[m[1]] = entry; count++; }
      } catch (_) {}
    }
    return count;
  }
  ...
  _load() {
    ...
    if (this._filePath && fs.existsSync(this._filePath)) {
      try { raw = JSON.parse(fs.readFileSync(this._filePath, 'utf8')); } catch (_) {}
    }
    if (raw) {
      ...
      this._nextPreKeyId            = _counter(raw.nextPreKeyId);
      this._firstUnuploadedPreKeyId = _counter(raw.firstUnuploadedPreKeyId);
    }

    // Files first — they are where a pre-key lives now.
    this._loadPreKeyFiles();

    // Anything still only in the old aggregate block moves out to a file of its
    // own. A session written before this change has its whole pool in there and
    // the server is serving those exact keys, so dropping them would leave
    // every bundle already handed out unanswerable.
    let migrated = 0;
    const legacy = (raw && raw.preKeys) || {};
    for (const id of Object.keys(legacy)) {
      if (this._preKeys[id]) continue;
      const entry = legacy[id];
      if (!entry || !entry.pubKey || !entry.privKey) continue;
      this._preKeys[id] = entry;
      this._pkPending.add(id);
      migrated++;
    }
    if (migrated) {
      this._flushPreKeysSync();
      this._save();   // rewrites the snapshot without the old block
    }

    // An id on disk is an id that has been used, whatever the counter says.
    const ids   = Object.keys(this._preKeys).map(Number).filter(n => Number.isFinite(n) && n > 0);
    const maxId = ids.length ? Math.max(...ids) : 0;
    if (this._nextPreKeyId <= maxId) this._nextPreKeyId = maxId + 1;
    if (this._firstUnuploadedPreKeyId > this._nextPreKeyId) {
      this._firstUnuploadedPreKeyId = this._nextPreKeyId;
    }
  }
  ...
  //
  // One-time pre-keys are not in here — they are a file each, see _preKeyFile().
  // Their two counters are, because they have to survive a pool that has been
  // drained to nothing and say what id comes next anyway.
  _snapshot() {
    return JSON.stringify({
      ...
      firstUnuploadedPreKeyId: this._firstUnuploadedPreKeyId
    });
  }

  // ─── Pre-key writing ──────────────────────────────────────────────────────

  // Mark one pre-key id as needing its file written (or removed, when the key
  // is no longer in memory) and arm the debounce.
  _savePreKey(id) {
    if (!this._pkDir) return;
    this._pkPending.add(String(id));
    if (this._pkTimer) clearTimeout(this._pkTimer);
    this._pkTimer = setTimeout(() => {
      this._pkTimer = null;
      this._flushPreKeysAsync();
    }, DEBOUNCE_MS);
  }

  _flushPreKeysAsync() {
    if (!this._pkDir || this._pkWriting || !this._pkPending.size) return;
    try { fs.mkdirSync(this._pkDir, { recursive: true }); } catch (_) { return; }

    this._pkWriting = true;
    const ids = [...this._pkPending];
    this._pkPending.clear();

    let next = 0, active = 0, done = false;
    const pump = () => {
      while (active < PRE_KEY_WRITE_CONCURRENCY && next < ids.length) {
        active++;
        this._writePreKeyAsync(ids[next++], () => { active--; pump(); });
      }
      if (!active && next >= ids.length && !done) {
        done = true;
        this._pkWriting = false;
        // Something came due while this batch was running — and cleared the
        // debounce that would have picked it up, since _savePreKey only arms a
        // timer. Arm one now.
        if (this._pkPending.size) this._savePreKey([...this._pkPending][0]);
      }
    };
    pump();
  }

  _writePreKeyAsync(id, cb) {
    if (!this._pkDir) return cb();
    const file  = this._preKeyFile(id);
    const entry = this._preKeys[id];
    if (!entry) return fs.unlink(file, () => cb());

    let data;
    try { data = _encodePreKeyFile(entry); } catch (_) { return cb(); }
    fs.writeFile(file, data, 'utf8', () => {
      // Consumed while the write was in flight. A key that has been used is one
      // the account must not offer again, so take it back off the disk rather
      // than leave the write that overtook the unlink standing.
      if (!this._preKeys[id]) return fs.unlink(file, () => cb());
      cb();
    });
  }

  _flushPreKeysSync() {
    if (!this._pkDir || !this._pkPending.size) return;
    if (this._pkTimer) { clearTimeout(this._pkTimer); this._pkTimer = null; }
    ...
    for (const id of ids) {
      ...
      const entry = this._preKeys[id];
      try {
        if (entry) fs.writeFileSync(file, _encodePreKeyFile(entry), 'utf8');
  ...
  _flushSync() {
    // Pre-keys have their own dirty set — a pool that changed while the
    // snapshot did not still has files owed to it.
    this._flushPreKeysSync();
  ...
  async storePreKey(keyId, keyPair) {
    ...
    this._savePreKey(id);
    // Storing an id spends it, whether it was generated here or restored from
    // somewhere else.
    this.setNextPreKeyId(Number(keyId) + 1);
  }

  async removePreKey(keyId) {
    delete this._preKeys[String(keyId)];
    this._savePreKey(keyId);
  }
  ...
  /** Whether this id is still in the pool — i.e. nobody has spent it yet. */
  hasPreKey(keyId) {
    return Object.prototype.hasOwnProperty.call(this._preKeys, String(keyId));
  }

  /** The id the next generated pre-key takes. Never goes backwards. */
  nextPreKeyId() {
    return this._nextPreKeyId;
  }

  setNextPreKeyId(id) {
    const n = Number(id);
    if (!Number.isFinite(n) || n <= this._nextPreKeyId) return;
    this._nextPreKeyId = Math.floor(n);
    if (this._firstUnuploadedPreKeyId > this._nextPreKeyId) {
      this._firstUnuploadedPreKeyId = this._nextPreKeyId;
    }
    this._save();
  ...
  /** The lowest id the server has not been sent. Never goes backwards. */
  firstUnuploadedPreKeyId() {
    return this._firstUnuploadedPreKeyId;
  }

  setFirstUnuploadedPreKeyId(id) {
    const n = Number(id);
    if (!Number.isFinite(n) || n <= this._firstUnuploadedPreKeyId) return;
    this._firstUnuploadedPreKeyId = Math.min(Math.floor(n), this._nextPreKeyId);
    this._save();
  }
}
