...
function deriveMessageKeys(messageKey) {
  ...
  const derived = Buffer.from(hkdf(sha256, messageKey, salt, info, 48));
  return {
...
class SenderKeyStore {
  ...
  _save() {
    ...
    this._saveTimer = setTimeout(() => {
      this._saveTimer = null;
      // The revision this write is a snapshot of.
      const rev  = this._rev;
      const data = JSON.stringify(this._data, null, 2);
      fs.mkdirSync(path.dirname(this._filePath), { recursive: true });
      // Only clear the flag when nothing changed while the write was running.
      // A mutation that arrived after the snapshot is not in `data`, and its
      // own debounce timer may never fire — clearing here would make the
      // exit-time _flushSync() skip it and lose it.
      fs.writeFile(this._filePath, data, 'utf8', () => {
