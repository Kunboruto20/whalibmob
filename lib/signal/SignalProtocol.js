...
// The pre-key pool, sized the way the reference client sizes it.
//
//   INITIAL_PREKEY_COUNT  what a fresh account puts on the server, and what it
//                         puts back when the server turns out to be holding
//                         none of ours
//   MIN_PREKEY_COUNT      the routine top-up, sent when the server says it is
//                         running low
//
// The pool is generated up front rather than on demand, so the authentication
// folder holds all 812 keys from the moment the session exists.
const INITIAL_PREKEY_COUNT = 812;
const MIN_PREKEY_COUNT     = 5;

// Below this many keys left, a session that is starting up refills to the full
// pool rather than waiting for the server to ask.
const PRE_KEY_LOW_WATER    = 50;
...
class SignalProtocol {
  ...
  _ensurePreKeys() {
    if (this.store.preKeyCount() >= PRE_KEY_LOW_WATER) return;
    this.replenishPreKeys();
  }

  /** Top the local pool back up to INITIAL_PREKEY_COUNT. */
  replenishPreKeys() {
    const need = INITIAL_PREKEY_COUNT - this.store.preKeyCount();
    if (need <= 0) return [];
    return this.generatePreKeys(need);
  }

  /** Generate `count` fresh pre-keys, continuing the id sequence. */
  ...
  /** The id of the newest pre-key generated, 0 when none ever was. */
  currentPreKeyId() {
    return this.store.nextPreKeyId() - 1;
  }

  hasPreKey(keyId) {
    return this.store.hasPreKey(keyId);
  }

  firstUnuploadedPreKeyId() {
    return this.store.firstUnuploadedPreKeyId();
  }

  /**
   * The next batch to send the server, generating whatever is missing to make
   * `count` of them — the same arithmetic the reference client does.
   *
   * `firstUnuploadedPreKeyId` is the lowest id the server has not been sent, so
   * the batch is the `count` ids from there up. Whatever of those has already
   * been generated is reused; the rest is made now, continuing from
   * `nextPreKeyId`. That is what stops an account from re-sending its whole
   * pool on every top-up — which is what this used to do, eight hundred keys at
   * a time, several times a session.
   *
   * The returned `update` is not applied here. Ids are spent the moment they
   * are generated (storePreKey advances nextPreKeyId), but a batch only counts
   * as uploaded once the server has acknowledged it — see commitPreKeyUpload().
   *
   * @returns {{preKeys: Array<{keyId, pubKey}>, update: {firstUnuploadedPreKeyId: number}}}
   */
  getNextPreKeys(count) {
    count = count > 0 ? Math.floor(count) : MIN_PREKEY_COUNT;

    const first     = this.store.firstUnuploadedPreKeyId();
    const available = this.store.nextPreKeyId() - first;
    const remaining = count - available;
    if (remaining > 0) this.generatePreKeys(remaining);

    const last = first + count - 1;
    const preKeys = [];
    for (let id = first; id <= last; id++) {
      const raw = this.store._preKeys[String(id)];
      // An id in the range whose key has already been consumed. The reference
      // client would put an undefined key on the wire here; skip it instead.
      if (!raw) continue;
      preKeys.push({ keyId: id, pubKey: Buffer.from(raw.pubKey, 'base64') });
    }

    return { preKeys, update: { firstUnuploadedPreKeyId: last + 1 } };
  }

  /** Record that the batch getNextPreKeys() returned reached the server. */
  commitPreKeyUpload(update) {
    if (!update) return;
    this.store.setFirstUnuploadedPreKeyId(update.firstUnuploadedPreKeyId);
  }

  /**
   * Every pre-key still in the pool, oldest id first.
   *
   * Used where a single key has to be picked out by id — the bundle attached to
   * a retry receipt — rather than for uploads, which go through
   * getNextPreKeys().
   */
  getPreKeysForUpload(max) {
    max = max || INITIAL_PREKEY_COUNT;
    const ids = Object.keys(this.store._preKeys)
      .map(Number)
  ...
  //
  // Uses WaSignalGroup GroupCipher/GroupSessionBuilder.
  // SenderKeyName format: groupId::senderId::0 (device 0 for group identity).

  _senderKeyName(groupId, senderId) {
    // A SenderKey belongs to a device, not to an account. Every device that
    // posts to a group distributes its own key, so the name has to carry the
    // device index — otherwise two devices of the same person share one slot
    // and the second one to speak overwrites the first, leaving its messages
    // undecryptable.
    //
    // Bare JIDs still resolve to device 0, so everything already on disk keeps
    // the exact same key string.
    const raw     = typeof senderId === 'string' ? senderId : String(senderId);
    const userPart = raw.split('@')[0];
    const colon    = userPart.indexOf(':');
    const user     = colon >= 0 ? userPart.slice(0, colon) : userPart;
    const deviceId = colon >= 0 ? (parseInt(userPart.slice(colon + 1), 10) || 0) : 0;
    return new SenderKeyName(groupId, { id: user, deviceId });
  }
...
module.exports = {
  SignalProtocol, jidToAddress, pad, unpad,
