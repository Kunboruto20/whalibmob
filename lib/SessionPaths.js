...
//
//   ~/.waSession/919634847671.json
//   ~/.waSession/919634847671.signal.json
//   ~/.waSession/919634847671.sk.json
...
//
//   <base>/919634847671/919634847671.json
//   <base>/919634847671/919634847671.signal.json
//   ...
//
...
// The one-time pre-keys are the exception to the list above. There are 812 of
// them and they are a file each — <phone>.pre-key-<id>.json for the number
// itself, <phone>.web.pre-key-<id>.json for its companion half — so they cannot
// be enumerated by suffix and have to be found by name instead.
const PRE_KEY_FILE_RE = (phone) =>
  new RegExp('^' + String(phone).replace(/\D/g, '') + '(\\.web)?\\.pre-key-\\d+\\.json$');

/**
 * Every pre-key file a number owns in a directory, by name.
 *
 * Anything that moves a session has to move these too: a number whose pre-keys
 * are left behind still has its Signal sessions and looks perfectly healthy
 * until somebody tries to open a new one with it, and then the pkmsg arrives
 * naming a key that is not there any more.
 */
function preKeyFilesFor(dir, phone) {
  const re = PRE_KEY_FILE_RE(phone);
  let entries;
  try { entries = fs.readdirSync(dir); } catch (_) { return []; }
  return entries.filter(name => re.test(name));
}
...
function migrateSession(baseDir, phone) {
  ...
  for (const name of preKeyFilesFor(baseDir, phone)) {
    const dest = path.join(to, name);
    if (_exists(dest)) { skipped.push(name); continue; }
    fs.renameSync(path.join(baseDir, name), dest);
    moved.push(name);
  }
