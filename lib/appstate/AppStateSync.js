...
function mutationKeys(keyData) {
  ...
  const e   = hkdfSha256(ikm, Buffer.alloc(0),
...
function decodeMutations(mutations, state, getKey, onMutation, validateMacs, opts) {
  ...
  for (const m of mutations) {
    // The index MAC is what the value is filed under in the hash, so a record
    // that arrived without one cannot be folded in at all — skip it the same
    // way as any other malformed row rather than let it take the run down.
    if (!m || !m.valueBlob || !m.keyId || !m.indexBlob || !m.indexBlob.length) {
      skipped++;
      continue;
    }
...
function decodePatches(name, patches, initial, getKey, getMutation, validateMacs) {
  ...
  for (const p of patches) {
    ...
    if (validateMacs && p.keyId) {
      ...
      if (!rec || !rec.keyData) throw new MissingAppStateKeyError(b64);
      const k    = mutationKeys(rec.keyData);
      // A mutation with no value blob has no value MAC to contribute, and the
      // server computed this patch MAC over every one of them. Leaving it out
      // is not a repair — the MAC below simply will not match, which is the
      // right answer for a patch we cannot reconstruct, and it is reached by
      // the ordinary macFailed path instead of by throwing mid-run.
      const macs = p.mutations
        .filter(m => m && m.valueBlob)
        .map(m => m.valueBlob.subarray(-32));
