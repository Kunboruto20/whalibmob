...
function extractMaterial(base, splits) {
  ...
  // Which about_logo.png, when there is more than one.
  //
  // A released bundle ships the drawable in every density split — seven of them
  ...
  // first path before any is searched for the second. The base APK keeps
  // priority within each path, since a release that still carries the drawable
  // itself is not a bundle at all.
  let aboutLogo = null;
  ...
  for (const p of ABOUT_LOGO_PATHS) {
    ...
    for (const split of (splits || [])) {
      ...
      aboutLogo = readZipEntry(split.data, readZipDirectory(split.data), p);
