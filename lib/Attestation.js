...
//
// Queries /cert?authKey=<identityPubHex>&enc=<encBase64>. The Frida server
// signs the ENC bytes with a fresh hardware-attested EC key and returns
// { signature, certificate }. signature → the &H= body suffix; certificate →
// the Authorization header (cert chain).
async function androidAttestBody(encB64, identityPubHex) {
  if (!isEnabled()) return { ...EMPTY_BODY };
  const res = await fridaGet('/cert', { authKey: identityPubHex, enc: encB64 });
...
// device          — { os }
// identityPubHex  — hex of the identity/noise public key (Android /cert authKey)
// noisePubB64     — base64 of the Noise public key (iOS clientDataHash source)
async function attestBody(encB64, device, keys) {
  keys = keys || {};
  if (device && device.os === 'android') {
    return androidAttestBody(encB64, keys.identityPubHex || '');
  }
