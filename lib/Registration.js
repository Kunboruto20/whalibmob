...
async function httpPostViaSocks(path, body, waVersion, proxyUrl, authHeader) {
  ...
  reqLines.push(`Connection: keep-alive`, '', body);
...
function getRequestVerificationCodeParameters(store, method, meta, device, attempt) {
  ...
  // iOS — matches Cobalt's IosClientRegistration.getRequestVerificationCodeParameters.
  // sim_mcc/sim_mnc are the literal "000", not the subscriber's real network:
  // an iPhone registering over SMS reports no SIM operator here, and sending a
  // real pair contradicts the rest of the profile.
  return [
    'method',            method,
    'sim_mcc',           '000',
    'sim_mnc',           '000',
