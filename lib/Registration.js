...
async function httpPostViaSocks(path, body, waVersion, proxyUrl, authHeader) {
  ...
  const headers  = registrationHeaders(getDeviceConfig(), waVersion);
  const reqLines = [
...
function loadAndroidMaterial() {
  ...
  try {
    ...
    throw new Error('registering as Android needs the token material from a WhatsApp APK, ' +
      'and there is none at ' + file + '. Extract it once per WhatsApp release with:\n' +
      '  wa apk-material <base.apk> [split.apk ...]\n' +
      'Pull the APK off a phone or emulator that has WhatsApp installed (adb), or point ' +
      'WA_ANDROID_APK_MATERIAL at a file you already have. Registering as iOS needs none of this.');
  }
  _androidMaterial = AndroidApk.materialFromJson(JSON.parse(raw));
...
function httpPost(path, body, waVersion, authHeader) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const headers = Object.assign(
      registrationHeaders(getDeviceConfig(), waVersion),
...
async function sendEncrypted(path, plaintext, store, waVersion) {
  ...
  const result = await httpPost(path, body, waVersion, bodyAtt.authorizationHeader);
...
module.exports._token = { computeToken, androidMaterialPath, registrationHeaders };
