...
async function requestSmsCode(store, method, opts) {
  ...
  // ── /exist, before /code ──────────────────────────────────────────────────
  //
  // The app never asks for a code without first checking the account slot, and
  // that check is where the session-start and exist_check funnel events come
  // from. A client that opens straight with /code has a shape the app never
  // produces.
  //
  // Skipped once a code has already been requested on this store, or once the
  // number is registered — re-checking there is not what the app does either,
  // and it would fire the session-start event a second time. This is the same
  // guard the CLI has always applied around its own call.
  //
  // Keys that are already spent do not fail with a network error, they come
  // back as a refusal, so this reports them as one rather than letting /code
  // spend one of the number's attempts learning the same thing. Regenerating
  // them is left to the caller: replacing the identity keys on a store handed
  // to us is not a decision this function gets to make quietly.
  if (opts.checkExist !== false && !store.codePending && !store.registered) {
    const fresh = await assertRegistrationKeys(store, waVersion);
    if (!fresh) {
      const err = new Error(
        'these device keys have already been used to register — WhatsApp answers ' +
        '/exist for them, so a /code request would spend one of the number\'s ' +
        'attempts and fail. Build a fresh store for this number (createNewStore / ' +
        'initAuthCreds) and request the code with that.');
      err.code = 'KEYS_ALREADY_REGISTERED';
      throw err;
    }
  }
