...
// Every country we carry metadata for has to be splittable here, or parsePhone
// takes the wrong prefix and the registration token ends up computed over the
// wrong national number — which the server answers by refusing the code. The
// two lists above were maintained by hand and had already drifted, so the
// country table itself gets the last word.
...
async function verifyCode(store, code, opts) {
  ...
  const normalized = normalizeCodeResult(code);
