...
// ---------- The version a connection announces ----------
//
// A session records the version it registered with, and announces that forever
// after. WhatsApp moves on, and a few months later the server refuses the
// handshake with 405 — a session that is perfectly valid, refused for saying it
// is an app that is too old. There was no way out of it either: the version
// lived in the session file, so the only remaining move was to register the
// number again, which is the one thing that should never answer a client-side
// problem.
//
// A real phone does not have this trouble. It registers once, WhatsApp updates,
// and it reconnects saying whatever it is now. Nothing ties the announced
// version to the registration: the registration token is signed over the APK's
// classes.dex and is never sent again after /register, so what a later
// connection calls itself is free to be current.
//
// So look it up, per platform, and take the newest of what is known.
...
// The newest of the candidates, ignoring anything that is not a dotted number.
function newestVersion(candidates) {
  let best = null;
  for (const c of candidates) {
    if (!c || !/^\d+(\.\d+)+$/.test(String(c))) continue;
    if (!best || compareVersions(c, best) > 0) best = String(c);
  }
  return best;
}

// Drop the memoised lookups, so the next call goes back to the network. Used
// when a handshake has just been refused over the version: whatever is cached
...
/**
 * The version a connection should announce for this session.
 *
 * WA_VERSION is the whole answer when it is set — no lookup, nothing compared,
 * nothing surprising. That is the point of pinning one.
 *
 * Otherwise it is the newest of: what the store registered with, what the
 * platform's store says today, and on Android the version of the APK the token
 * material came from. Android gets both because either can be the stale one —
 * material downloaded weeks ago goes behind, and the Play listing is scraped
 * out of a web page that can stop parsing — and between the two, one is
 * generally current.
 *
 * Never throws and never blocks on a lookup that will not answer: a failed
 * fetch simply does not contribute a candidate, which leaves the session's own
 * version, exactly as before this existed. Offline behaviour is unchanged.
 */
async function resolveConnectVersion(store) {
  if (process.env.WA_VERSION) return process.env.WA_VERSION;

  const device     = (store && store.device) || getDeviceConfig();
  const candidates = [];
  if (store && store.version) candidates.push(store.version);

  if (device.os === 'android') {
    const material = tryLoadAndroidMaterial();
    if (material && material.apkVersion) candidates.push(material.apkVersion);
  }

  try {
    candidates.push(device.os === 'android'
      ? await fetchAndroidVersion()
      : await fetchIosVersion());
  } catch (_) { /* the lookup is an opinion, not a requirement */ }

  return newestVersion(candidates) || (store && store.version) || null;
}
...
module.exports = { checkIfRegistered, checkNumberStatus, requestSmsCode, verifyCode, fetchIosVersion, fetchAndroidVersion, fetchWaVersion, parsePhone, getCountryMeta, assertRegistrationKeys, parseSocksProxy, socksProxyUrl,
  resolveConnectVersion, clearVersionCache, compareVersions, newestVersion };
