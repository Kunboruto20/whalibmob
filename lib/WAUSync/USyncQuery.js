...
class USyncQuery {
    ...
    parseUSyncQueryResult(result) {
        ...
        const parseUsers = (listNode) => {
            if (!listNode || !Array.isArray(listNode.content)) return [];
            return listNode.content.reduce((acc, node) => {
        ...
        if (usyncNode) {
            queryResult.list = parseUsers(findChild(usyncNode, 'list'));
            // side_list carries users too and is a normal part of a result —
            // leaving it unparsed makes callers iterating sideList see nothing.
            queryResult.sideList = parseUsers(findChild(usyncNode, 'side_list'));
        }
