      ...
      try {
        ...
        if (err && /\b421\b/.test(err.message)) {
          ...
          const members = this._client._getGroupMembers
            ...
            : [];
          // LID members are cached under the "lid:" key, same as the DM 421
          // handler and the phash-mismatch flush build it — the bare user
          // would miss the entry entirely.
          this._devMgr.clearCache(members.map(jid =>
