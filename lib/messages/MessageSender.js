...
class MessageSender {
  ...
  async _sendGroupMessage(groupJid, msgId, plaintext, mediaType, options) {
    ...
    // A pkmsg must travel with the account's signed device identity — it is how
    // the recipient verifies the new Signal session. Emitting one without it
    // gets the whole stanza rejected, which surfaces only as an ack error.
    if (skdmHasPkmsg && !advBytes) {
      throw new Error(
        'Cannot send to ' + groupJid + ': the SenderKey distribution needs a pkmsg ' +
        'but this session has no device identity (advIdentity is empty — the server ' +
        'did not supply device-identity on <success>). Reconnect to obtain it.'
      );
    }
