...
class MessageSender {
  ...
  async _sendDMMessage(toJid, msgId, plaintext, mediaType, options) {
    ...
    // keeps the stanza addressed to the JID the caller passed:
    //     const finalJid = jid;
    //     const destinationJid = !isStatus ? finalJid : statusJid;
    // It never rewrites `to` to the LID. The LID is used only to look up
    // devices, fetch prekeys and address the per-device <to> entries; the
    // envelope stays on the phone JID. Rewriting the envelope is what draws
    // 479, which both whatsmeow #859 and the Baileys LID reports describe as
    // appearing specifically when the message is addressed to an @lid.
    //
    // WA_DM_ROUTE=lid restores the old envelope-on-LID behaviour for
    // comparison; anything else keeps the Baileys shape.
    ...
    // Participants still come from the LID device list whenever we know it —
    // that part matches Baileys too, which encodes @lid per-device JIDs while
    // leaving the envelope alone.
    const useLidRoute  = !!lidUser;
    const routingToJid = (lidUser && routeMode === 'lid') ? `${lidUser}@lid` : toJid;

    _whaDbg('[DBG] DM_ROUTE phone=' + recipientPhone +
      ' envelope=' + routingToJid + ' participants=' +
      (useLidRoute ? 'LID' : 'PN') + ' mode=' + routeMode);
