...
function deriveMediaKeyData(mediaKey, keyName) {
  ...
  return hkdfSha256(ikm, Buffer.alloc(0), info, EXPANDED_SIZE);
}
...
function resolveMediaUrl(url, directPath) {
  if (url) return String(url);
  if (!directPath) return null;
  const p = String(directPath);
...
//
// A file that has moved answers with a redirect, sometimes more than one. A
// loop, a redirect with nowhere to go, and an outright refusal all have to end
// as a rejection: an error page handed back as if it were the media only turns
// up later as a MAC failure in decryptMedia, which says nothing about why.
  ...
  return new Promise((resolve, reject) => {
    ...
    lib.get(url, { agent: agent || undefined, headers: mediaHeaders(web) }, (res) => {
      ...
      if (status === 301 || status === 302) {
        ...
        if (!location) {
          return reject(new Error('httpGet: the server answered ' + status +
            ' without a Location header'));
        }
        const hops = (depth || 0) + 1;
        if (hops > MAX_REDIRECTS) {
          return reject(new Error('httpGet: more than ' + MAX_REDIRECTS +
            ' redirects'));
        }
        return _httpGet(location, opts, hops).then(resolve, reject);
      }
      if (status < 200 || status > 299) {
        res.resume();                                   // an error page, not media
        return reject(new Error('httpGet: the server answered ' + status));
      }
      const chunks = [];
      res.on('data', c => chunks.push(c));
