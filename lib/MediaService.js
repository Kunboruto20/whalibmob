...
async function uploadMedia(mediaType, data, hosts, auth, opts) {
  opts = opts || {};
  ...
  if (!hosts || hosts.length === 0) {
    throw new Error('No media hosts available (media connection not established)');
  }

  let lastErr = null;
  for (const host of hosts) {
    try {
      const result = await _tryUpload(host, typeInfo.path, token, authEncoded, encrypted, timeoutMs);
      if (result && (result.url || result.direct_path || result.directPath)) {
        return {
          url:          result.url        || '',
      ...
      lastErr = new Error('empty upload response from ' + (typeof host === 'string' ? host : host.hostname));
    } catch (err) {
      lastErr = err;
    }
  }

  throw new Error('Media upload failed on all hosts: ' + (lastErr ? lastErr.message : 'unknown'));
}

function _tryUpload(host, mediaPath, token, auth, encrypted, timeoutMs) {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode !== 200) {
          return done(reject, new Error(`HTTP ${res.statusCode} from ${hostname}`));
        }
        try {
          done(resolve, JSON.parse(Buffer.concat(chunks).toString('utf8')));
        } catch (_) {
          done(reject, new Error('invalid JSON from ' + hostname));
        }
      });
    });

    // Fail fast instead of hanging on a stale media host / expired auth token —
    // a hung upload surfaces to the user as "connection expired".
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`upload timeout after ${timeoutMs}ms on ${hostname}`));
    });
