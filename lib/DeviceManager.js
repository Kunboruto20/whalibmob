          ...
          if (deviceListNode && Array.isArray(deviceListNode.content)) {
            for (const devNode of deviceListNode.content) {
              ...
              const devId = devNode.attrs && devNode.attrs.id != null
                ...
                : 0;
              const keyIdx = devNode.attrs && devNode.attrs['key-index'] != null
                ? parseInt(String(devNode.attrs['key-index']), 10)
                : -1;
              // Only include non-zero devices that have a valid key-index (mirrors Baileys extractDeviceJids)
              if (devId === 0 || keyIdx > 0) {
        ...
        for (const userNode of listNode.content) {
          ...
          if (deviceListNode && Array.isArray(deviceListNode.content)) {
            for (const devNode of deviceListNode.content) {
              ...
              const keyIdx = devNode.attrs && devNode.attrs['key-index'] != null
                ? parseInt(String(devNode.attrs['key-index']), 10) : -1;
              if (devId === 0 || keyIdx > 0) {
