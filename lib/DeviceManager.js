...
class DeviceManager {
  constructor(client) {
    ...
    //
    // The option is `stdTTL`. It was `ttl`, which is the name of the per-entry
    // argument to set(), not of the default in the constructor — an unknown
    // option is simply ignored, so the cache was built with no expiry at all
    // and a contact's device list, once learned, was never asked about again
    // for the life of the connection. Someone who linked WhatsApp Web or a
    // desktop while the client was up stayed invisible: their new device was
    // left out of every stanza, and a stanza that does not cover a recipient's
    // current devices is refused by the server.
    this._deviceCache   = new NodeCache({ stdTTL: DEVICE_CACHE_TTL });
  ...
  // Bulk read. Answers with a plain object holding only the keys that are live
  // in the cache — the same set _dcHas() answers true for, empty-array values
  // included. One call instead of one per key, which is what a group send
  // spends most of its cache time on: a 200-member group used to ask 200 times
  // whether it had to go to the network, and then 200 times again for the
  // values it had just been told were there.
  _dcMGet(keys) {
    if (!keys.length) return {};
    if (typeof this._deviceCache.mget === 'function') {
      return this._deviceCache.mget(keys) || {};
    }
    const out = {};
    for (const k of keys) {
      if (this._deviceCache.has(k)) out[k] = this._deviceCache.get(k);
    }
    return out;
  }

  // Bulk write, and the snapshot kept in step so the on-disk cache still
  // mirrors what is in memory.
  //
  // The per-entry option is `value`. node-cache's own documentation calls it
  // `val`, and that name is accepted and then ignored: the key is created with
  // no value at all, so has() answers true while get() answers undefined. That
  // reads exactly like "cached, and this contact owns no devices", which drops
  // every device they have from the next stanza. Same shape of trap as the
  // `ttl`/`stdTTL` one above, and just as silent — hence the spelling here.
  _dcMSet(entries) {
    if (!entries.length) return;
    if (typeof this._deviceCache.mset === 'function') {
      this._deviceCache.mset(entries.map(([key, value]) => ({ key, value })));
    } else {
      for (const [key, value] of entries) this._deviceCache.set(key, value);
    }
    for (const [key, value] of entries) this._cacheSnapshot[key] = value;
  }
  _dcFlush() {
  ...
  _saveToDisk() {
    ...
    try {
      // The snapshot mirrors the cache on every write, but nothing tells it
      // when an entry expires — so once entries actually start expiring it
      // keeps them, and writing it out unfiltered would put a device list on
      // disk after the cache had already decided it was too old to trust. The
      // next start reads that file back in, and the expiry never happens.
      //
      // Ask the cache what it still holds. `keys()` is no use here: it lists
      // expired entries too, until the periodic sweep gets to them. `has()`
      // answers for the entry itself.
      const entries = {};
      for (const key of Object.keys(this._cacheSnapshot)) {
        if (this._deviceCache.has(key)) entries[key] = this._cacheSnapshot[key];
        else delete this._cacheSnapshot[key];
      }
  ...
  async _usyncGetDevices(phones) {
    ...
    //
    // Read the whole set in one go and hand the result straight to
    // _jidsFromCache, so a fully-cached group costs two cache calls rather than
    // two per member.
    const cached   = this._dcMGet(phones);
    const uncached = phones.filter(p => !Object.prototype.hasOwnProperty.call(cached, p));
    if (uncached.length === 0) {
      return this._jidsFromCache(phones, cached);
    }
  ...
  //
  // `prefetched` is the result of a _dcMGet() the caller has already paid for.
  // Only pass it when nothing can have changed since — after an IQ the cache
  // has been rewritten and the values have to be read again.
  _jidsFromCache(phones, prefetched) {
    const cached = prefetched || this._dcMGet(phones);
    const jids = [];
    for (const p of phones) {
      // Same answer _dcGet() gives: a missing key reads as null, an empty
      // array stays an empty array.
      const devices = cached[p] || null;
  ...
  async _doUsyncIq(phones) {
    ...
    //
    // Bulk, for the same reason the read above is: in a large group the phones
    // the response said nothing about are often most of them, and this used to
    // be a has() and a set() each.
    const present  = this._dcMGet(phones);
    const fallback = phones
      .filter(p => !Object.prototype.hasOwnProperty.call(present, p))
      .map(p => [p, [0]]);
    if (fallback.length) {
      this._dcMSet(fallback);
      _whaDbg('[USYNC_FALLBACK] phones=' + fallback.map(([p]) => p).join(','));
    }
    this._scheduleSave();
  }
  ...
  async ensureOwnLidDeviceSessions(lidUser, ownPhone, signalProto, allowPkmsg = true) {
    ...
    // Same rule as the phone-addressed path: skip the device we are, not the
    // device that happens to be numbered zero.
    const selfDevice = this._ownDeviceIndex();
    const deviceIds  = (this._dcGet(cacheKey) || []).filter(d => d !== selfDevice);
    const deviceJids = deviceIds.map(d => makeDeviceJid(lidUser, d, 'lid'));
  ...
  //
  // Which device index this client occupies.
  //
  // A device registered over SMS is the account's primary and is always 0. A
  // companion linked by pairing code occupies a numbered slot — and then
  // device 0 is the owner's phone, which is a recipient like any other.
  _ownDeviceIndex() {
    const store = this._client && this._client._store;
    return (store && store.deviceIndex) || 0;
  }

  async ensureOwnDeviceSessions(ownPhone, signalProto, allowPkmsg = true) {
    // Everything the account owns except the device doing the sending.
    //
    // This used to drop device 0 unconditionally, which is right only while we
    // are device 0. As a companion it silently excluded the owner's own phone
    // from every message we sent: the recipient got it, the phone did not, and
    // it sat there showing "waiting for this message" forever. It also put our
    // participant list out of step with the server's, which is what the phash
    // mismatch on the ack was reporting.
    const selfDevice = this._ownDeviceIndex();
    ...
    if (this._ownDeviceJids !== null && Date.now() < this._ownDeviceExpiry) {
      const others = this._ownDeviceJids.filter(j => stripUser(j).device !== selfDevice);
    ...
    const others = deviceJids.filter(j => stripUser(j).device !== selfDevice);
    return this._ensureSessions(others, signalProto, allowPkmsg);
  }
