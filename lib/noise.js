...
const CONNECT_REASON_ERROR_RECONNECT  = 3;
  ...
  _handleHandshakeFrame(frame) {
    try {
      ...
      const payload = encodeClientPayload({
        ...
        // ConnectType is a closed set — 0 CELLULAR_UNKNOWN, 1 WIFI_UNKNOWN, and
        // 100-112 for the named cellular radios. There is no 3, which is what a
        // reconnect used to send.
        connectType:        CONNECT_TYPE_WIFI,
        // ConnectReason: 1 USER_ACTIVATED for a connect somebody asked for,
        // 3 ERROR_RECONNECT for the ones the retry loop makes. It had always
        // claimed the first, including on its own automatic retries.
        connectReason:      _reconnect ? CONNECT_REASON_ERROR_RECONNECT
                                       : CONNECT_REASON_USER_ACTIVATED,
        ...
        userAgent: {
          ...
          // Build.MODEL, which is what the registration User-Agent already sent
          // as `Device/<manufacturer>-<modelId>`. Announcing the marketing name
          // here made the two describe different handsets for the same session.
          // iOS registers with the marketing name, so it keeps sending it.
          device:          _isAndroid ? _device.modelId : _device.model,
          osBuildNumber:   _device.osBuildNumber,
          // Same case the registration sent it in — lowercase on Android,
          // uppercase on iOS (Registration.js). The account was registered
          // against one form of this id; the connect announced the other.
          phoneId:         _isAndroid ? this.store.fdid.toLowerCase()
                                      : this.store.fdid.toUpperCase(),
          releaseChannel:  0,
