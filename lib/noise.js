    ...
    try {
      ...
      if (this._awaitingAuth) {
        ...
        if (tag === 'failure') {
          ...
          const EXPLAIN = {
            '401': 'the account logged this device out. The registration is ' +
                   'gone server-side — nothing in the session file can fix it, ' +
                   'it has to be registered again. Using the same number on ' +
                   'the real WhatsApp app is the usual cause.',
            '403': 'the account is blocked from third-party clients.',
            '405': 'the server refused this client — usually the announced ' +
                   'app version.',
            '409': 'the number is logged in somewhere else.',
            '500': 'the session state is corrupt; register again.'
          };
